    ...
    order(obj={}){
        ...
        let self = this;
            ...
            Promise.allSettled(poolPromises)
                ...
                let iteration_request = obj.request_id +'_'+ obj.iteration_id;               
                //self.sendSrcParsePageData(poolResponce, iteration_request);
...
//------------------------------------------------------------------------------
    async exec(url, obj){
        ...
        return this.request.get(url, {timeout: 1000}, proxy, description);
    }
...
//------------------------------------------------------------------------------
    async sendSrcParsePageData(sendData, iteration_request, obclass=false){
        ...
        const send = await obclass.request.post(`${obclass.settings.hostController}receive-parse-data`, sendData).then(function(res) {
           // //////console.log(res);
            if(res.status == true){
          //     //////console.log('send_api_success: '+iteration_request, 'code: '+res.code); 
            }
            else{
           //     //////console.log('send_api_error: '+iteration_request, 'code: '+res.code);
      ...
      //  console.log(JSON.stringify({razd:'**********************************************************************************',optionPost:options,dataPost:data}));
        const req = http.request(options, res => {
            ...
            if(res.statusCode >= 200 && res.statusCode < 400){
                // console.log('send_api_success ', 'code: '+res.statusCode);
            }
            else{
                console.log('send_api_error ', 'code: '+res.statusCode);
                //setTimeout(obclass.simplePost, obclass.randomNumber(100,1000), dataSend, obclass);
            }
        });
        ...
        try{
            const browser = await puppeteer.launch({
                ...
                args: [
                    ...
                    `--proxy-server=${proxyData.proxy.proxy_host}:${proxyData.proxy.proxy_port}`
                ]
            });
             
                const page = await browser.newPage();
                await page.authenticate({ username: proxyData.proxy.service_user, password: proxyData.proxy.service_password });          
                await page.goto(url,{
        ...
        try{    
            for await(const [num, promise] of Object.entries(poolRresponce) ) {         
                if (promise.status == "fulfilled") {
                    ...
                    // set Descriptions
                    self.fillDescriptions(rez, description);

                    // simple props
                    let defHtml = await page.content();
                    self.fillParseData(rez, defHtml, obj.search);
                    ...
                    if(obj.hasOwnProperty('browserSearch') && Object.entries(obj.browserSearch).length > 0){
                        for (const [type, obType] of Object.entries(obj.browserSearch) ) {
                            switch (type) {
                                case 'lazyBlock':  // from lazy props
                                    self.fillParseFromLazy(rez, defHtml, obType);
                                break;
                                case 'itemTableSizes':  // from click sizes table
            ...
            //////console.log('SEND ERROR FROM processingGoodsResponce');
            //////console.log(e);
        }    
...
//------------------------------------------------------------------------------
    async fillParseFromTableSizes(source, page, params, type){
        ...
        try{
            ...
            const $ = cheerio.load(contentSizesTable, {normalizeWhitespace: true,decodeEntities: false});
            let sizesTable = {};
        ...
        for (let url of obj.input) {
            ...
            try{
                ...
                if(pageRez.code >= 200 && pageRez.code < 400){
                    ...
                    let defHtml = await page.content();
                    self.fillParseData(pageRez, defHtml, obj.search);
