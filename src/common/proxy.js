    ...
    async   setProxyList(){
     ...
     let tmpPids=await this.settings.runProcess();
        ...
        if(tmp.status==true && tmp.response.error==0){
          ...
          this.log.success({message:'Получен новый прокси лист'});
        }
        else{
           this.ProxyList=[];
           this.index=0;
          this.log.error({message:'Произошла ошибка при получении прокси листа'});
        }
    return this;
   ...
   return new Promise( (resolve, reject)=>{
       ...
       if(count<=0){
         reject({status:0});  
           ...
           resolve({status:1,proxy:self.ProxyList[self.index]});
